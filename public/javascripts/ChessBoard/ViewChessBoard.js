...
export class ViewChessBoard {
  constructor(clickChessPiece, clickEmptyCell) {
    ...
    this.chessBoardCells = [...this.dom.chessBoard.rows].map(row =>
  ...
  renderSaveGame(saveGame) {
    return saveGame.arrChessPieces.map(el => {
      ...
      if (el.pos.x == null) {
        el.color == "white"
          ? this.dom.whiteOut.append(el.div)
          : this.dom.blackOut.append(el.div);
  ...
  moveToEmptyCell(chessPiece, emptyCell) {
    ...
    chessPiece.pos = {
      x: emptyCell.cellIndex,
      y: emptyCell.parentNode.rowIndex
    };
  }
  ...
  takingEnemyChessPiece({ first, second }, enPassant = false) {
    ...
    second.color == "white"
      ? this.dom.whiteOut.append(second.div)
      : this.dom.blackOut.append(second.div);
    second.pos.x = null;
    second.pos.y = null;
  ...
  kingIsBlinking(whiteKing, blackKing) {
    let king = whiteKing.div.classList.contains("figure_kill")
      ? whiteKing
      : blackKing;
